...
): z.infer<typeof import("@lore/shared").Stat> {
  return {
    kind,
    size: stats.size,
...
export async function stat(
  relPath: string
): Promise<z.infer<typeof import("@lore/shared").Stat>> {
  const filePath = resolveWorkspacePath(relPath);
...
export async function readdir(
  relPath: string,
  opts?: z.infer<typeof ReaddirOptions>
): Promise<z.infer<typeof DirEntry>[]> {
  const dirPath = resolveWorkspacePath(relPath);
  const entries: z.infer<typeof DirEntry>[] = [];
  ...
  async function readDir(
    ...
    for (const item of items) {
      ...
      let itemKind: z.infer<typeof NodeKind>;
...
export async function readFile(
  relPath: string,
  encoding: z.infer<typeof Encoding> = "utf8"
): Promise<z.infer<typeof ReadFileResult>> {
  const filePath = resolveWorkspacePath(relPath);
  const stats = await fs.lstat(filePath);
